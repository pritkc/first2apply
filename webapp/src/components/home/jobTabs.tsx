...
export function JobTabs() {
  ...
  const siteIds = searchParams.get("site_ids")
    ? searchParams.get("site_ids")?.split(",").map(Number) ?? []
    : [];
  const linkIds = searchParams.get("link_ids")
    ? searchParams.get("link_ids")?.split(",").map(Number) ?? []
    : [];
  ...
  return (
    <Tabs value={status} onValueChange={(value: string) => onTabChange(value)}>
